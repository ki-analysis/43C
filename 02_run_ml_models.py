...
from ml_models import gdbt_selector_pipeline, gdbt_selector_grid
...
def run_all_models(
    df, output_covariate, input_covariates_list, names_of_covariate_groups, groups=None,
):
    # some kids don't have various measurements, so we have to drop them
    df = drop_the_right_rows(
        df,
        output_covariate,
        covariates_to_check=input_covariates_list[names_of_covariate_groups.index("similarityscores")],
    )
    ...
    results["GBDT"] = sklearn_pipeline_evaluator(
        df,
        output_covariate,
        input_covariates_list,
        names_of_covariate_groups,
        gdbt_selector_pipeline,
        gdbt_selector_grid,
        groups=None,
        outer_cv=outer_cv,
        random_state=RANDOM_STATE,
    ...
    df_displays = []
    print(f"{output_covariate} Results")
    print("--------------------------------------------------")
...
if __name__ == "__main__":
    ...
    for output in crypto_output_6_covariates:
        all_results[output] = run_all_models(
            df_crypto, output, crypto_input_6_covariates, crypto_names_of_covariate_groups,
        )

    # 24m - crypto
    for output in crypto_output_24_covariates:
        all_results[output] = run_all_models(
            df_crypto, output, crypto_input_24_covariates, crypto_names_of_covariate_groups,
        )

    # 36m - provide
    for output in provide_output_36_covariates:
        all_results[output] = run_all_models(
            df_provide, output, provide_input_36_covariates, provide_names_of_covariate_groups,
        )

    # 60m - provide
    for output in provide_output_60_covariates:
        all_results[output] = run_all_models(
            df_provide, output, provide_input_60_covariates, provide_names_of_covariate_groups,
        )
