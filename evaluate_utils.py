...
from ml_models import meta_pipeline, meta_grid, SELECTOR_PERCENTILE
    ...
    if shuffle_split or n_splits == 1:
        if groups:
            cv = GroupShuffleSplit(
                ...
                test_size=shuffle_split_train_size,
                random_state=random_state,
            )
...
def stacked_model(
    ...
    for feat_name, feat_list in zip(names_of_covariate_groups, input_covariates_list):
        # if there's too few features, we can't take too small of a percentage of them
        if "selector__percentile" in grid:
            # don't bother selecting features when there's too few of them
            if len(feat_list) <= 10:
                grid["selector__percentile"] = [100]
            else:
                grid["selector__percentile"] = []
                for selector_percentile in SELECTOR_PERCENTILE:
                    if np.floor(len(feat_list) * selector_percentile / 100) > 1:
                        grid["selector__percentile"].append(selector_percentile)
    ...
    clf_stacked = StackingClassifier(estimators=clfs, final_estimator=meta_clf, cv=inner_cv, n_jobs=n_jobs, verbose=0)
